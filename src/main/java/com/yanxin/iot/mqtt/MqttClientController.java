...
import com.yanxin.iot.mariadb.beans.SensorPO;
import com.yanxin.iot.mariadb.dbop.DBop;
...
public class MqttClientController implements MqttCallback {
   ...
   * @param password the password for the user
	...
	public void messageArrived(String topic, MqttMessage message) throws MqttException {
		...
		DevicePayload devicePayload = jsonParser.getGson().fromJson(jsonPayload, DevicePayload.class);
		...
		cachedThreadPool.execute(new Runnable() {
			public void run() {
					SensorPO sensor = DBop.getSensorById(devicePayload.getDeviceId());

			        if(null != sensor) {
			        	
			        	SensorDataPO sensorData = new SensorDataPO(sensor, data);
			        	
			        	DBop.insertSensorData(sensorData);
			            log.info("Successfully get sensor by ID!");
			        }else {
			        	log.warn(" I think it's a invalid sensor with id = "+devicePayload.getDeviceId()+", and please regist it first!");
			        }
